...
object BatchMergeWithSelfInput {
  private val MODEL_TYPE: String = "model_type"
  private val INPUT_TABLES: String = "input_tables"
  private val OUTPUT_TABLE: String = "output_table"
  ...
  private var updatedModelToTables: Map[String, Array[String]] = modelToTables

  def main(args: Array[String]): Unit = {
    val parameter = ParametersTool.fromArgs(args)
    var modelType = parameter.get(MODEL_TYPE)
    val inputTables = parameter.get(INPUT_TABLES)
    val outputTable = parameter.get(OUTPUT_TABLE)
    val beginDay = parameter.get(BEGIN_TIME)
    val endDay = parameter.get(END_DAY)
    val batchSize = parameter.getInt(BATCH_SIZE, 10)
    if (modelType == null && outputTable == null) {
      println("=" * 50)
      println(MODEL_TYPE + " or " + INPUT_TABLES + " 、 " + OUTPUT_TABLE + " is request! ")
      println("=" * 50)
      System.exit(1)
    }
    if (inputTables != null && outputTable != null) {
    ...
    if (!updatedModelToTables.contains(modelType)) {
      println("*" * 50)
