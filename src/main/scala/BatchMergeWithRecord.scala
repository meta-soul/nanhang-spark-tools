...
//    "111" -> Array("file:/Users/dudongfeng/work/zehy/lakesoul/test_table", "default.test_table_1", "default.test_table_2"),
...
//    val tableName = "qarfile_merge_record"
    ...
    rows.foreach(row => {
      ...
      for (i <- 1 to tableArray.length - 1) {
        ...
        val sql = s"select * from $tableName where flt_dt = '$fltDt' and tail_num = '$tailNum' and file_no = '$fileNo'"
        println(sql)
        val data = spark.sql(sql).toDF()
        sinkTable.upsert(data)
      }
      import spark.implicits._
      val sqlInsert = Seq((modelType, fltDt, tailNum, fileNo)).toDF("model_type", "flt_dt", "tail_num", "file_no")
  ...
  private def getPartitions(inputTableName: String, dayList: util.List[String]): util.ArrayList[Row] = {
    ...
    System.out.println("============= PartitionInfo size: " + inputTablePartitionInfo.size + " =============")
    val inputTablePartitionStrList = new util.ArrayList[String]
    System.out.println("============= day list is:" + dayList.toString + " ============= ")
    ...
    System.out.println("============= inputTablePartition list size filter days: " + inputTablePartitionStrList.size + " =============")
